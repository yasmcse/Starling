...
import com.challenge.designsystem.theme.Black50
...
class AddNewGoalFragment : Fragment() {
    ...
    private val viewModel by viewModels<AddNewGoalViewModel>()
    private val args: AddNewGoalFragmentArgs by navArgs()
                    ...
                    Screens.SAVING_GOALS -> {
                        with(drawerState) {
                            with(starlingNavigator) {
                                coroutineScope.launch {
                                    if (isOpen) close()
                                    navigateToSavingsGoalsScreen(args.addNewSavingGoal)
                                }
                            }
                        }
                    }
    ...
    fun LoadScreen() {
        ...
        var targetCurrency: String by remember { mutableStateOf("") }
        ...
        val responseModel by viewModel.response.collectAsStateWithLifecycle()
            ...
            Column(
                ...
                Box(
                    ...
                    TextField(
                        ...
                        label = {
                            Text(
                                text = stringResource(R.string.currency_e_g_gbp),
                                style = MaterialTheme.typography.caption,
                                color = Color.Black
                            )
                        },
                ...
                networkStatus.hasNetworkAccess(activity as FragmentActivity) -> {
                    when (responseModel) {
                        ...
                        is NetworkResult.Error -> {
                            Toast.makeText(
                                context,
                                responseModel.errorResponse?.errors?.get(0)?.message,
                                Toast.LENGTH_LONG
                            ).show()
                        }
                    }
                    ...
                    Toast.makeText(
                        context, com.challenge.common.R.string.no_internet, Toast.LENGTH_SHORT
                    ).show()
                }
            }
        }
